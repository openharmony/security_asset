    ...
    match fs::read(path) {
        ...
        Err(e) => {
            macros_lib::log_throw_error!(macros_lib::hisysevent::function!(),
                ErrCode::FileOperationError, "[FATAL][SA]Read database key ciphertext failed! error is [{}]", e )
        },
    }
}
...
pub fn write_db_key_cipher(user_id: i32, db_key_cipher: &[u8]) -> Result<()> {
    ...
    match fs::write(path, db_key_cipher) {
        ...
        Err(e) => {
            macros_lib::log_throw_error!(macros_lib::hisysevent::function!(),
                ErrCode::FileOperationError, "[FATAL][SA]Write database key ciphertext failed! error is [{}]", e )
        },
    }
}
        ...
        match fs::remove_file(file.path().to_string_lossy().to_string()) {
            ...
            Err(e) => {
                return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(),
                    ...
                    "[FATAL]Remove [{}] failed, error code:[{}]", file.path().to_string_lossy().to_string(), e )
            },
        }
    }
    Ok(())
