...
use saf_definition::{
    macros_lib, SAFError, SAFMap, Conversion, DataType, ErrCode, Result, Tag, Value
};
...
pub fn serialize_map(map: &SAFMap, parcel: &mut MsgParcel) -> Result<()> {
    if map.len() as u32 > MAX_MAP_CAPACITY {
        return macros_lib::log_throw_error!(ErrCode::ParamVerificationFailed, "[FATAL][IPC]The map size exceeds the limit.");
    }
...
pub fn serialize_maps(vec: &Vec<SAFMap>, parcel: &mut MsgParcel) -> Result<()> {
    if vec.len() as u32 > MAX_VEC_CAPACITY {
        return macros_lib::log_throw_error!(ErrCode::ParamVerificationFailed, "[FATAL][IPC]The vector size exceeds the limit.");
    }
