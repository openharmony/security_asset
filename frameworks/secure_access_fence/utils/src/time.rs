...
fn get_timespec() -> Result<Timespec> {
    ...
    if unsafe { clock_gettime(CLOCK_REALTIME, &mut ts) } != 0 {
        return macros_lib::log_throw_error!(ErrCode::GeneralError, "clock_gettime failed");
    }
    if ts.tv_sec < 0 {
        return macros_lib::log_throw_error!(ErrCode::GeneralError, "32-bit time_t overflow (Y2038)");
    }
    Ok(ts)
}
...
pub fn system_time_in_millis() -> Result<u64> {
    ...
    let sec_millis = (ts.tv_sec as i64)
        .checked_mul(SECS_TO_MILLIS)
        .ok_or_else(|| macros_lib::log_and_into_saf_error!(
            ErrCode::GeneralError,
            "seconds to milliseconds overflow"
        ))?;
    let nano_millis = ts.tv_nsec as i64 / NANOS_TO_MILLIS;
    let millis = sec_millis
        .checked_add(nano_millis)
        .ok_or_else(|| macros_lib::log_and_into_saf_error!(
            ErrCode::GeneralError,
    ...
    if millis < 0 {
        return macros_lib::log_throw_error!(
            ErrCode::GeneralError,
            "milliseconds is negative: {}", millis
        );
    }
    Ok(millis as u64)
}
