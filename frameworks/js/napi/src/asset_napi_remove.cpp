...
napi_value NapiRemove(const napi_env env, napi_callback_info info, bool asUser, bool async)
{
...
{
    return NapiRemove(env, info, false, true);
}

napi_value NapiRemoveAsUser(const napi_env env, napi_callback_info info)
{
    return NapiRemove(env, info, true, true);
}
