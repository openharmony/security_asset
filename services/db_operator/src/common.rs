...
mod operation_add_common;

pub use argument_check::{
    check_group_validity, check_required_tags, check_tag_validity, check_value_validity, MAX_LABEL_SIZE,
};
pub use permission_check::check_system_permission;
pub use operation_add_common::*;
...
pub(crate) const TAG_COLUMN_TABLE: [(Tag, &str); 21] = [
...
/// Defines the critical label attributes.
pub const CRITICAL_LABEL_ATTRS: [Tag; 4] =
    [Tag::DataLabelCritical1, Tag::DataLabelCritical2, Tag::DataLabelCritical3, Tag::DataLabelCritical4];

/// Defines the normal label attributes.
pub const NORMAL_LABEL_ATTRS: [Tag; 4] =
    [Tag::DataLabelNormal1, Tag::DataLabelNormal2, Tag::DataLabelNormal3, Tag::DataLabelNormal4];

/// Defines the normal local label attributes.
pub const NORMAL_LOCAL_LABEL_ATTRS: [Tag; 4] =
    [Tag::DataLabelNormalLocal1, Tag::DataLabelNormalLocal2, Tag::DataLabelNormalLocal3, Tag::DataLabelNormalLocal4];

/// Defines the access control attributes.
pub const ACCESS_CONTROL_ATTRS: [Tag; 10] = [
    Tag::Alias,
    Tag::Accessibility,
    Tag::AuthType,
    Tag::IsPersistent,
...
/// Transfer the asset map into a db map.
pub fn into_db_map(attrs: &AssetMap) -> DbMap {
...
/// Transfer the db map into an asset map.
pub fn into_asset_map(db_data: &DbMap) -> AssetMap {
...
/// Add owner info into the db map.
pub fn add_owner_info(calling_info: &CallingInfo, db_data: &mut DbMap) {
...
/// Add group info into the db map.
pub fn add_group(calling_info: &CallingInfo, db_data: &mut DbMap) {
    if let Some(group) = calling_info.group() {
        db_data.insert(column::GROUP_ID, Value::Bytes(group));
    };
}

/// Add calling info into the db map.
pub fn add_calling_info(calling_info: &CallingInfo, db_data: &mut DbMap) {
...
/// Build aad.
pub fn build_aad(attrs: &DbMap) -> Result<Vec<u8>> {
    let mut aad = Vec::new();
