...
fn check_array_size(tag: &Tag, value: &Value, min: usize, max: usize) -> Result<()> {
    let Value::Bytes(v) = value else {
        return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::InvalidArgument, "[FATAL][{}] is not a bytes.", tag);
    };
...
fn check_enum_variant<T: TryFrom<u32>>(tag: &Tag, value: &Value) -> Result<()> {
    let Value::Number(n) = value else {
        return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::InvalidArgument, "[FATAL][{}] is not a number.", tag);
    };
...
fn check_user_id(tag: &Tag, value: &Value) -> Result<()> {
    ...
    match is_user_id_exist(*n as i32) {
        Ok(res) if res => Ok(()),
        Ok(_) => macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::InvalidArgument, "[FATAL]The user id [{}] is not exist.", *n),
        Err(e) => Err(e),
    }
}
...
pub fn check_required_tags(attrs: &AssetMap, required_tags: &[Tag]) -> Result<()> {
    for tag in required_tags {
        if !attrs.contains_key(tag) {
            return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::InvalidArgument, "[FATAL]The required tag [{}] is missing.", tag);
        }
    }
    Ok(())
}
