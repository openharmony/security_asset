...
fn get_remain_partition_size(partition: &str) -> Result<f64> {
    ...
    if ret_code != 0 {
        return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::try_from(ret_code as u32)?, "Get remain partition size failed");
    }
    Ok(remain_size)
}
...
fn get_db_dirs() -> Result<Vec<String>> {
    ...
    if ret != SUCCESS {
        return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::AccountError, "[FATAL] Get users IDs failed.");
    }
