...
pub fn upgrade_single_clone_app_data(user_id: i32, hap_info: String) -> Result<()> {
    ...
    if parts.len() < INIT_INDEX + 1 {
        return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::InvalidArgument, "Hap info too short.");
    }
    match parts.last().unwrap().parse::<i32>() {
        ...
        Err(_) => return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::InvalidArgument, "Upgrade clone app failed."),
    }
...
fn get_clone_app_indexes(user_id: i32, app_name: &str) -> Result<Vec<i32>> {
    ...
    let app_name_cstr = match CString::new(app_name) {
        ...
        Err(_) => return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::OutOfMemory, "Create CString failed."),
    };
    ...
    if ret != SUCCESS {
        return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::try_from(ret as u32)?, "Get clone app indexes failed.");
    }
...
fn is_hap_in_allowlist(user_id: i32, info: &str) -> Result<bool> {
    ...
    if ret != SUCCESS {
        return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::try_from(ret as u32)?, "Check hap in allowlist failed.");
    }
    Ok(is_in_list)
}
...
fn clone_single_app(user_id: i32, app_name: &str, app_index: i32, datas: &mut Vec<DbMap>) -> Result<()> {
    ...
    let index = match owner_info.iter().rev().position(|&x| x == b'_') {
        Some(index) => index,
        _ => return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::InvalidArgument, "Owner info is incorrect."),
    };
    if index >= owner_info.len() - 1 {
        return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::InvalidArgument, "Owner info is too short.");
    }
    ...
    if need_rollback {
        ...
        return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::DatabaseError, "Upgrade clone app data failed.");
    }
    ...
    match load.process_event(EventType::WrapData, &mut params) {
        Ok(()) => {
            ...
            if db_clone.insert_datas(unwrap_info.data).is_err() {
                return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::CryptoError, "Unwrap the clone app data failed.");
            }
        },
        Err(_) => {
            return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::CryptoError, "Unwrap the clone app data failed.");
