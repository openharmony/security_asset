...
fn check_attrs_array(attributes_array: &[AssetMap]) -> Result<()> {
    for attrs in attributes_array {
        check_required_tags(attrs, &QUERY_VALID_ATTRS).map_err(|e| macros_lib::track_error!(e, macros_lib::hisysevent::function!()))?;
...
fn check_update_array(attributes_array: &[AssetMap]) -> Result<()> {
    for attrs in attributes_array {
        ...
        check_tag_validity(attrs, &valid_tags).map_err(|e| macros_lib::track_error!(e, macros_lib::hisysevent::function!()))?;
...
fn local_batch_update(
    ...
    let attributes = match attributes_array.first() {
        Some(attr) => attr,
        None => return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::InvalidArgument, "[FATAL]Batch Update argument empty."),
    };
    ...
    check_update_array(attributes_to_update_array).map_err(|e| macros_lib::track_error!(e, macros_lib::hisysevent::function!()))?;
    ...
    db.update_batch_datas(&db_map, attributes_array, attributes_to_update_array, calling_info).map_err(|e| macros_lib::track_error!(e, macros_lib::hisysevent::function!()))
}
...
pub(crate) fn batch_update(
    ...
    || attributes_array.len() != attributes_to_update_array.len(){
        return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::InvalidArgument, "[FATAL]Batch Update argument empty.");
    }
    local_batch_update(calling_info, attributes_array, attributes_to_update_array).map_err(|e| macros_lib::track_error!(e, macros_lib::hisysevent::function!()))
}
