...
fn check_arguments(query: &AssetMap, attrs_to_update: &AssetMap, calling_info: &CallingInfo) -> Result<()> {
    ...
    common::check_required_tags(query, &QUERY_REQUIRED_ATTRS).map_err(|e| macros_lib::track_error!(e, macros_lib::hisysevent::function!()))?;
    ...
    if attrs_to_update.is_empty() {
        return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::InvalidArgument, "[FATAL]The attributes to update is empty.");
    }
    ...
    common::check_tag_validity(attrs_to_update, &valid_tags).map_err(|e| macros_lib::track_error!(e, macros_lib::hisysevent::function!()))?;
    common::check_value_validity(attrs_to_update).map_err(|e| macros_lib::track_error!(e, macros_lib::hisysevent::function!()))
}
...
pub(crate) fn update(calling_info: &CallingInfo, query: &AssetMap, update: &AssetMap) -> Result<()> {
    check_arguments(query, update, calling_info).map_err(|e| macros_lib::track_error!(e, macros_lib::hisysevent::function!()))?;
    ...
    add_attrs(update, &mut update_db_data).map_err(|e| macros_lib::track_error!(e, macros_lib::hisysevent::function!()))?;
    ...
    let results = db.query_datas(&vec![], &query_db_data, None, true).map_err(|e| macros_lib::track_error!(e, macros_lib::hisysevent::function!()))?;
    if results.is_empty() {
        return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::NotFound, "[FATAL]The asset to update is not found.");
    }

    if update.contains_key(&Tag::Secret) {
        let mut results = db.query_datas(&vec![], &query_db_data, None, true).map_err(|e| macros_lib::track_error!(e, macros_lib::hisysevent::function!()))?;
        if results.len() != 1 {
        ...
        let cipher = encrypt(calling_info, result).map_err(|e| macros_lib::track_error!(e, macros_lib::hisysevent::function!()))?;
        update_db_data.insert(column::SECRET, Value::Bytes(cipher));
    }

    // call sql to update
    let update_num = db.update_datas(&query_db_data, true, &update_db_data).map_err(|e| macros_lib::track_error!(e, macros_lib::hisysevent::function!()))?;
    if update_num == 0 {
        return macros_lib::log_throw_error!(macros_lib::hisysevent::function!(), ErrCode::NotFound, "[FATAL]Update asset failed, update 0 asset.");
    } else {
