...
fn query_non_exist_without_alias() {
    ...
    expect_error_eq(ErrCode::NotFound, asset_sdk::Manager::build().unwrap().lock().unwrap().query(&query).unwrap_err());
}
...
fn query_without_alias_with_wrong_condition() {
    ...
    asset_sdk::Manager::build().unwrap().lock().unwrap().add(&add).unwrap();
...
fn query_without_limit() {
    ...
    assert_eq!(asset_num, asset_sdk::Manager::build().unwrap().lock().unwrap().query(&query).unwrap().len() as u32);
...
fn query_with_offset_without_limit() {
    ...
    let offset = 3;
    let query = AssetMap::from([(Tag::ReturnOffset, Value::Number(offset))]);
    assert_eq!(asset_num - offset, asset_sdk::Manager::build().unwrap().lock().unwrap().query(&query).unwrap().len() as u32);
...
fn query_with_limit_with_without_offset() {
    ...
    let assets = asset_sdk::Manager::build().unwrap().lock().unwrap().query(&query).unwrap();
...
fn query_with_secret() {
    ...
    expect_error_eq(ErrCode::InvalidArgument, asset_sdk::Manager::build().unwrap().lock().unwrap().query(&query).unwrap_err());
...
fn query_with_return_all_without_alias() {
    ...
    expect_error_eq(ErrCode::Unsupported, asset_sdk::Manager::build().unwrap().lock().unwrap().query(&query).unwrap_err());
...
fn query_with_wrong_auth_token() {
    ...
    expect_error_eq(ErrCode::AccessDenied, asset_sdk::Manager::build().unwrap().lock().unwrap().query(&query).unwrap_err());
...
fn query_with_return_order_by() {
    ...
    let mut query = AssetMap::new();
    query.insert_attr(Tag::ReturnOrderedBy, Tag::DataLabelNormal1 as u32);
    let mut assets = asset_sdk::Manager::build().unwrap().lock().unwrap().query(&query).unwrap();
    ...
    assets = asset_sdk::Manager::build().unwrap().lock().unwrap().query(&query).unwrap();
    assert_eq!(expect_query_len, assets.len() as u32);
    assert!(!assets[0].get_bytes_attr(&Tag::DataLabelNormal1).unwrap().eq(NORMAL_LABEL1));
