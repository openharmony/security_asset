...
{
    printf("run here and attrCnt is: %d \n", attrCnt);
    ...
    AssetAttr attr[] = {
        {
            ...
            .tag = ASSET_SYSTEM_TAG_USER_ID,
            .value.u32 = SPECIFIC_USER_ID
        }
    };
...
int32_t QueryByAlias(const char *alias, AssetResultSet *resultSet)
{
    AssetAttr attr[] = {
        ...
        { .tag = ASSET_SYSTEM_TAG_USER_ID, .value.u32 = SPECIFIC_USER_ID }
    };
...
HWTEST_F(AssetSystemApiTest, AssetSystemApiTest001, TestSize.Level0)
{
    ...
    ASSERT_EQ(true, checkMatchAttrResult(attr2, ARRAY_SIZE(attr2), &result));
    printf("run here 4\n");
