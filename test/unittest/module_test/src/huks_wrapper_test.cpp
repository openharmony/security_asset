...
HWTEST_F(HuksWrapperTest, HuksWrapperTest001, TestSize.Level0)
{
    ...
    ASSERT_EQ(ASSET_SYSTEM_SUCCESS, GenerateKey(&keyId, true, false));
    ASSERT_EQ(ASSET_SYSTEM_SUCCESS, IsKeyExist(&keyId));
    ASSERT_EQ(ASSET_SYSTEM_SUCCESS, DeleteKey(&keyId));
}
...
HWTEST_F(HuksWrapperTest, HuksWrapperTest002, TestSize.Level0)
{
    ...
    struct HksBlob keyAlias = { (uint32_t)strlen(tmpKeyAlias), (uint8_t *)tmpKeyAlias };
    struct KeyId keyId = { 0, keyAlias, DEVICE_POWERED_ON };
    ASSERT_EQ(ASSET_SYSTEM_SUCCESS, GenerateKey(&keyId, false, false));
    ...
    ASSERT_EQ(ASSET_SYSTEM_SUCCESS, EncryptData(&keyId, &aadData, &inData, &outData));
    ASSERT_EQ(ASSET_SYSTEM_SUCCESS, DecryptData(&keyId, &aadData, &outData, &inData));
    ASSERT_EQ(ASSET_SYSTEM_SUCCESS, DeleteKey(&keyId));
}
...
HWTEST_F(HuksWrapperTest, HuksWrapperTest003, TestSize.Level0)
{
    ...
    ASSERT_EQ(ASSET_SYSTEM_SUCCESS, InitKey(&keyId, 600, &challengeData, &handleData));

    uint8_t authToken[148] = { 0 };
    struct HksBlob authTokenData = { 148, authToken };
    /* auth token is not ok, result in update&finish fail */
    ASSERT_EQ(ASSET_SYSTEM_ACCESS_DENIED, ExecCrypt(&handleData, &aadData, &authTokenData, &outData, &inData));
    ASSERT_EQ(ASSET_SYSTEM_NOT_FOUND, Drop(&handleData));
    ASSERT_EQ(ASSET_SYSTEM_SUCCESS, DeleteKey(&keyId));
}
